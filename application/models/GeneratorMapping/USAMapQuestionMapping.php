 ...
 * @package models
 */
class Model_GeneratorMapping_USAMapQuestionMapping extends Model_GeneratorMapping_AbstractImageQuestionMapping {
	...
	protected function createQuestionTitle() {
		$question = 'Auf welchem US-Staat steht das rote Fähnchen?';
	...
	protected function createFakeAnswers() {
		$key = $this->key -1;
		$numberOne = ($key +2 > 49) ? $key -2 : $key +2;
		$key = $this->key -1;
		$numberTwo = ($key -1 < 0) ? $key +3 : $key -1;
		$key = $this->key -1;
		$numberThree = ($key +4 > 49) ? $key -3 : $key +4;
	...
	protected function createCategories() {
		$this->categoriesData = array(1,10,11); 
	}
