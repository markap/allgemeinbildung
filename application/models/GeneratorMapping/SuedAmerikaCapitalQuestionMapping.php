...
/**
 * mapping suedamerika - capital 
 *
 * @package models
 */
class Model_GeneratorMapping_SuedAmerikaCapitalQuestionMapping extends Model_GeneratorMapping_AbstractImageQuestionMapping {
	...
	protected function createFakeAnswers() {
		$key = $this->key -1;
		$numberOne = ($key +2 > 14) ? $key -2 : $key +2;
		$key = $this->key -1;
		$numberTwo = ($key -1 < 0) ? $key +3 : $key -1;
		$key = $this->key -1;
		$numberThree = ($key +4 > 14) ? $key -3 : $key +4;
